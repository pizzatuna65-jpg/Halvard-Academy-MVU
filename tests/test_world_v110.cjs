...
console.log('Sleep quality, bond cap, Dove cover');
{
  ...
  const bond = (d, loc, gain, S = S0) => applyPatch(at(S, d, '13:00', loc), [{ op: 'delta', path: '/Bonds/Etnie/Progress', value: gain }]).Bonds.Etnie.Progress;
  ...
  ok(bond(dull, 'Canteen', 5) === 3, 'normal daily cap 3');
  ok(bond(rainAft, 'Canteen', 5) === 4, 'stuck indoors together in the rain: cap 4');
  ok(bond(fineAft, 'Gardens', 5) === 4, 'outdoors in fine mild weather: cap 4');
  ok(bond(rainAft, 'Canteen', 9) === 4, 'never more than +1');
  ok(bond(rainAft, 'Canteen', 5, applyPatch(S0, [{ op: 'replace', path: '/$ui/wxfx', value: 'flavor' }])) === 3, 'Flavor: no weather bonus');
