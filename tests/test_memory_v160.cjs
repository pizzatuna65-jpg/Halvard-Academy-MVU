...
P('09:00', [here(['Irene', 'Royhan'])]);
S.Bonds.Irene.Rank = 3; S.Bonds.Royhan.Rank = 3;
...
// ---- Imprints (1.6.3: Irene is "fixed" by the G1 canon, so the replacement rules are shown on Royhan, who is "shaped" (Royhan became "fixed" in 1.6.6))
const imp = (b, w) => ins('Royhan', 'Imprints', { Belief: b, Weight: w, From: 'test' });
P('10:00', [imp('Small thing', 3)]);
ok(!S.Bonds.Royhan.Imprints.length && /needs weight 5 or more/.test(S._Log.join(' ')), 'an Imprint under weight 5 is refused');
P('10:05', [imp('A', 5), imp('B', 6), imp('C', 7), imp('D', 8), imp('E', 9)]);
ok(S.Bonds.Royhan.Imprints.length === 5 && S.Bonds.Royhan.Imprints[0].When === 'M1 W1 Mon', 'five Imprints, dated');
P('10:10', [imp('F', 5)]);
ok(S.Bonds.Royhan.Imprints.length === 5 && !S.Bonds.Royhan.Imprints.some(x => x.Belief === 'F') && /does not outweigh the lightest/.test(S._Log.join(' ')), 'a sixth that does not outweigh the lightest is refused');
P('10:15', [imp('G', 10)]);
const G = S.Bonds.Royhan.Imprints.find(x => x.Belief === 'G');
ok(S.Bonds.Royhan.Imprints.length === 5 && G && G.Was === 'A' && !S.Bonds.Royhan.Imprints.some(x => x.Belief === 'A'), 'a heavier one replaces the lightest and keeps what it replaced (Was)');
{ // a "fixed" character (data/npc_canon.json Change) is never rewritten: same engine with Royhan fixed
  const src = rd('src/scripts/engine.js').replace(/const CHANGE = \{[^\n]*\};/, "const CHANGE = {\"Royhan\":\"fixed\"};");
  const run = new Function(src + '\nreturn runEngine;')();
  const T = Schema.parse(_.cloneDeep(S)); T.Bonds.Royhan.Imprints.push({ Belief: 'H', Weight: 10, When: '', From: '', Was: '' }); run(T, S, '');
  ok(!T.Bonds.Royhan.Imprints.some(x => x.Belief === 'H') && /does not change at the core \(fixed\)/.test(T._Log.join(' ')), 'a fixed character: a sixth Imprint is refused however heavy');
}
...
// ---- Cast Sheet and <now>
S = applyPatch(S, [here(['Irene', 'Royhan', 'Aiden', 'Castor', 'Kanae'])]);
let t = R(509, S);
